<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="应聘部门" prop="yingpinbumen">
          <el-select v-model="ruleForm.yingpinbumen" placeholder="请选择应聘部门">
            <el-option
                v-for="(item,index) in yingpinbumenOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="应聘部门" prop="yingpinbumen">
	      <el-input v-model="ruleForm.yingpinbumen"
                placeholder="应聘部门" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="应聘职务" prop="yingpinzhiwu">
          <el-select v-model="ruleForm.yingpinzhiwu" placeholder="请选择应聘职务">
            <el-option
                v-for="(item,index) in yingpinzhiwuOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="应聘职务" prop="yingpinzhiwu">
	      <el-input v-model="ruleForm.yingpinzhiwu"
                placeholder="应聘职务" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.yuangongxingming = true;
          ...
          if(o=='yingpinbumen'){
            this.ruleForm.yingpinbumen = obj[o];
	    this.ro.yingpinbumen = true;
            continue;
          }
          if(o=='yingpinzhiwu'){
            this.ruleForm.yingpinzhiwu = obj[o];
	    this.ro.yingpinzhiwu = true;
            continue;
          }
          if(o=='fasongshijian'){
		...
		if(json.yuangongxingming!=''&&json.yuangongxingming){
          ...
          this.$message.error(data.msg);
            ...
            this.$http({
              ...
              if (data && data.code === 0) {
                this.yingpinbumenOptions = data.data;
              } else {
                this.$message.error(data.msg);
              }
            });
            ...
            this.$http({
              ...
              if (data && data.code === 0) {
                this.yingpinzhiwuOptions = data.data;
              } else {
                this.$message.error(data.msg);
              }
            });
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `yuangongyingpin/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.yuangongyingpinCrossAddOrUpdateFlag = false;
                  this.parent.search();
