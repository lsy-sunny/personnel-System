  ...
  <div>
    <div class="container">
      <div class="login-form" style="backgroundColor:rgba(229, 238, 229, 0.07);borderRadius:10px">
        <h1 class="h1" style="color:rgba(24, 103, 228, 1);fontSize:28px;">人事管理系统注册</h1>
		<el-form ref="rgsForm" class="rgs-form" :model="rgsForm" label-width="120px">
			...
			<el-form-item label="用户姓名" class="input" v-if="tableName=='putongyonghu'">
			...
			<el-form-item label="手机" class="input" v-if="tableName=='putongyonghu'">
			...
			<el-form-item label="邮箱" class="input" v-if="tableName=='putongyonghu'">
...
export default {
  ...
  methods: {
    ...
    login() {
      ...
      if((!this.ruleForm.yonghuxingming) && `putongyonghu` == this.tableName){
        this.$message.error(`用户姓名不能为空`);
        return
      }
      if(`putongyonghu` == this.tableName && this.ruleForm.shouji&&(!this.$validate.isMobile(this.ruleForm.shouji))){
        this.$message.error(`手机应输入手机格式`);
        return
      }
      if(`putongyonghu` == this.tableName && this.ruleForm.youxiang&&(!this.$validate.isEmail(this.ruleForm.youxiang))){
